@foreach ($notifications as $notification)
    @if ($notification->notification_type == config('custom.call'))
        <a href="{{ route('user.show', $notification->userSend->id) }}" class="{{ $notification->viewed == 0 ? 'not-viewed' : '' }}" data-id="{{ $notification->id }}">
            <div class="btn btn-danger btn-circle"><i class="fa fa-phone"></i></div>
            <div class="mail-contnet" style="width: 95%">
                <h5>{{ config('custom.call') }}</h5> 
                <span class="mail-desc"></span> @lang('custom.noty_call_with', ['User' => $notification->userSend->name])
                <span class="time" style="color: black;">{{ \Carbon\Carbon::parse($notification->created_at)->diffForHumans() }}</span>
            </div>
        </a>
